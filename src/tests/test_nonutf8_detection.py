...
def test_no_multibyte_document_is_mislabeled_as_utf8(corpus):
    """No non-UTF-8 multibyte document may be reported as UTF-8.

    This is the exact regression fixed in the overlay: without the language
    pass the UTF-8 prober never rejected invalid byte sequences, so GBK/
    EUC-KR/... text scored as UTF-8. A UTF-8 mislabel is the dangerous case --
    a caller's open(encoding="utf-8") then mojibakes or raises.
    """
    offenders = []
...
def test_multibyte_detection_is_accurate(corpus):
    """Every multibyte document must decode-match its source codec.

    Genuine multibyte detections score well above the overlay's 0.5 threshold,
    so accuracy here is effectively 100%; require >= 99% to leave headroom for
    an incidental ambiguous sample without letting a real regression slip by.
    """
    total = correct = 0
    for data, source_codec in zip(corpus["documents"], corpus["expected"]):
        if source_codec not in _MULTIBYTE_CODECS:
...
def test_issue_33_style_false_positive_is_suppressed(corpus):
    """No document may be confidently mislabeled Big5 (issue #33 class).

    The near-ASCII Windows-1252 sample in issue #33 was detected as Big5 at
    ~0.4 confidence once the bogus UTF-8 candidate was removed. The overlay's
    0.5 multibyte threshold suppresses that; guard the corpus against any
    non-Big5 document coming back as a low-confidence Big5.
    """
    for data, source_codec in zip(corpus["documents"], corpus["expected"]):
