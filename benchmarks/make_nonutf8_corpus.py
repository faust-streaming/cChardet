...
UTF-8 fast path answers before the detection engine (the path that regressed
in freedesktop uchardet #38, and the one prone to mislabeling non-UTF-8 bytes
as UTF-8) is ever exercised. This builds a labeled corpus of non-UTF-8
documents so both throughput and accuracy can be regression-gated, with no
network access or SHA pinning.

It writes a pickled dict of parallel lists: "documents" (raw encoded bytes)
and "expected" (the source codec of each). Correctness is judged by
decode-equivalence, not exact label match, tolerating benign aliasing
(GBK/GB18030, EUC-KR/UHC, ISO-8859-1/-15) while still rejecting a UTF-8
mislabel, whose bytes do not decode as UTF-8 at all.
"""
